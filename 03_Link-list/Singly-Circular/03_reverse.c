...
void reverse(){
    if(head == NULL){
        ...
        temp = head ;
        currentnode = nextnode = head ; // currentnode & nextnode is pointing to 1st node initially
        do{ // this becomes true when the next node is head node
            nextnode = currentnode->next ; // shifting nextnode one by one node to right
            currentnode->next = prev ; // linking the node in a reverse order using prev
            prev = currentnode ; // updating prev in reverse order
            currentnode = nextnode ;// also shifting currentnode to right by one by one node
        }while(currentnode != head);

        head->next = prev ; // connecting just made last node with just made 1st node 
        head = prev ; // making head to the last node as our goal
    }
...
void displayReverse(){
    ...
    do{
        ...
        // prev = prev->next ; // shifting prev to left one by one node
        temp = temp->next ; // shifting temp one node by node
        i--; // to print the order in orignal node no. i.e; 3->2->1
    }while(temp != head);
}
...
while(0 != 1){
    ...
    printf("Press : 5 ---> Reverse the list.\n");
    printf("Press : 6 ---> Display the reverse of the list.\n");
